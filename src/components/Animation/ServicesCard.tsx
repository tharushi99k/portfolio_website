...
const ServicesCard: React.FC<Props> = ({ num, title, descrip }) => {
    ...
    return (
        <div className='custom_service border border-[#01F191] z-[100] relative transform rounded-2xl text-center p-4 shadow-md w-full sm:w-80 md:w-full mx-auto'>
            <IconComponent className='w-10 h-20 text-[#01F191] mx-auto' />
            <h1 className='text-sm sm:text-sm md:text-base text-white mt-4'>
                {title}
            </h1>
            <p className='text-white text-sm sm:text-sm md:text-sm opacity-60 mt-3'>
                {descrip}
            </p>
